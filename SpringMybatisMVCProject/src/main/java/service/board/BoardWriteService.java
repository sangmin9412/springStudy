...
public class BoardWriteService {
	...
	public void boardInsert(BoardCommand boardCommand, HttpSession session, HttpServletRequest request) {
		BoardDTO dto = new BoardDTO();
		AuthInfo authInfo = (AuthInfo) session.getAttribute("authInfo");
		dto.setUserId(authInfo.getUserId());
		dto.setBoardName(boardCommand.getBoardName());
		dto.setBoardPass(bcryptPasswordEncoder.encode(boardCommand.getBoardPass()));
